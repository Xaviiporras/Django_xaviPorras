...
def teacher_detail(request, teacher_id):
    if 1 <= teacher_id <= len(profes):
        profe = profes[teacher_id - 1]
        return render(request, 'teacher_detail.html', {'profe': profe})

def students(request):
    return render(request, 'students.html', {'alumnes': alumnes})
def student_detail(request, student_id):
    if 1 <= student_id <= len(alumnes):
        alumne = alumnes[student_id - 1]
        return render(request, 'student_detail.html', {'alumne': alumne})
